...
- **KolibriScript Interpreter** — модуль `backend/src/script.c`, выполняющий сценарии с директивами «обучить», «спросить», «похвала», «порицание» и связывающий их с REPL-командой `:script путь`.
...
- **KolibriScript Interpreter** executes scripted directives (`обучить`, `спросить`, `похвала`, `порицание`) and is exposed via the `:script <path>` REPL command.
...
- **KolibriScript Interpreter**：执行包含“обучить / спросить / похвала / порицание”指令的脚本，可通过 `:script <path>` 命令触发。
