...
- **Chat/REPL** — русскоязычный интерфейс команд `:teach`, `:ask`, `:tick`, `:evolve`, `:why`, `:canvas`, `:sync`, `:cluster list`, `:cluster broadcast`, `:cluster hello`, `:verify`, который транслирует действия в цифровой геном и поддерживает распределённое обучение.
...
- **Оркестрация технической готовности.** Скрипт `scripts/run_all.sh` объединяет сборку, тесты, генерацию `kolibri.wasm`/`kolibri.iso` и (по желанию) запуск локального роя, фиксируя отсутствие внешних зависимостей предупреждениями. Это упростило повторяемые прогоны при подготовке релизов.
...
6. Сформирована инфраструктура технической готовности: `scripts/build_iso.sh`, `scripts/build_wasm.sh`, `scripts/run_all.sh`, цель `make check` и релизные заметки фиксируют контроль ISO/WASM и CI.
