...
2. **Обучить базовый словарь приветствий и ответов.**
   - Использовать `scripts/experiments/llm_teacher.py` (в режиме HTTP или `torch`) с системной подсказкой «Отвечай на русском языке», чтобы загрузить в память Kolibri стартовые пары `вопрос → ответ` («привет», «как дела», «кто ты», и т.д.).
