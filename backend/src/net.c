...
size_t kn_message_encode_formula(uint8_t *buffer, size_t buffer_len,
  ...
  offset += sizeof(be_b);
  memcpy(payload + offset, &fitness_bits,
         sizeof(fitness_bits));

  size_t header = kolibri_write_header(buffer, buffer_len,
                                       KOLIBRI_MSG_MIGRATE_RULE,
                                       sizeof(payload));
  if (header == 0 || buffer_len < header + sizeof(payload)) {
    return 0;
  }
...
int kn_message_decode(const uint8_t *buffer, size_t buffer_len,
  ...
  case KOLIBRI_MSG_MIGRATE_RULE: {
    ...
    memcpy(&a_raw, payload + offset, sizeof(a_raw));
    offset += sizeof(a_raw);
    memcpy(&b_raw, payload + offset, sizeof(b_raw));
    offset += sizeof(b_raw);
