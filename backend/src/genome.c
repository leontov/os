...
static int preobrazovat_stroku_v_cifry(const char *istochnik, uint8_t *naznachenie,
        uint16_t *dlina, size_t emkost)
{
    ...
    if (kolibri_transducirovat_utf8(&potok,
                (const unsigned char *)istochnik, dlina_bajt) != 0) {
        return -1;
    }
...
static int preobrazovat_ascii_v_cifry(const char *istochnik, uint8_t *naznachenie,
        uint16_t *dlina, size_t emkost)
{
    if (!istochnik || !naznachenie || !dlina) {
        return -1;
        ...
        ReasonBlock *blok)
{
    ...
    if (preobrazovat_ascii_v_cifry(sobytie, blok->event_digits,
                &blok->event_digits_len, KOLIBRI_EVENT_DIGITS) != 0) {
        return -1;
    }
    if (preobrazovat_ascii_v_cifry(nagruzka, blok->payload_digits,
                &blok->payload_digits_len, KOLIBRI_PAYLOAD_DIGITS) != 0) {
        return -1;
    }
        ...
        ReasonBlock *vyhod)
{
    ...
    unsigned char bufer[sizeof(blok.index) + sizeof(blok.timestamp) +
            KOLIBRI_HASH_SIZE + sizeof(blok.event_digits_len) + KOLIBRI_EVENT_DIGITS +
            sizeof(blok.payload_digits_len) + KOLIBRI_PAYLOAD_DIGITS];
    size_t dlina_bufera = 0U;
    sobrat_paket(&blok, bufer, &dlina_bufera);
    ...
    if (preobrazovat_cifry_v_stroku(blok.event_digits, blok.event_digits_len,
                sobytie_ascii, sizeof(sobytie_ascii)) != 0) {
        return -1;
    }
    if (preobrazovat_cifry_v_stroku(blok.payload_digits, blok.payload_digits_len,
                nagruzka_ascii, sizeof(nagruzka_ascii)) != 0) {
        return -1;
    }
    ...
    int zapisano = fprintf(kontekst->file, "%llu,%llu,%s,%s,%s,%s\n",
            ...
            pred_hex, hmac_hex, sobytie_ascii, nagruzka_ascii);
    if (zapisano < 0) {
        return -1;
    }
...
int kg_block_event_text(const ReasonBlock *blok, char *bufer, size_t razmer)
{
    ...
    potok.poziciya = 0U;
    size_t zapisano = 0U;
    if (kolibri_izluchit_utf8(&potok, (unsigned char *)bufer, razmer, &zapisano) != 0) {
        return -1;
    }
    if (zapisano >= razmer) {
        return -1;
